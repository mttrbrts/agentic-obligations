...
import os from 'os';
import { Introspector, ModelManager, Factory, Serializer, ModelUtil } from '@accordproject/concerto-core';
import dayjs from 'dayjs';
import utc from 'dayjs/plugin/utc';
import { ToWords } from 'to-words';
import { TemplateMarkModel, ConcertoMetaModel, CommonMarkModel, CiceroMarkModel } from '@accordproject/markdown-common';
...
import { templatemarkutil, TemplateMarkTransformer } from '@accordproject/markdown-template';
...
function nameUserCode(templateMarkDom) {
  return traverse(templateMarkDom).map(function(x) {
    if (x && (x.$class === `${TemplateMarkModel.NAMESPACE}.ConditionalDefinition` && x.condition || x.$class === `${TemplateMarkModel.NAMESPACE}.ClauseDefinition` && x.condition)) {
      ...
      if (WITH_DEFINITION_RE.test(nodeClass)) {
        ...
        if (typeof data === "object") {
          ...
          if (variableValues.length === 0) {
            ...
            const type = ModelUtil.isPrimitiveType(context.elementType) ? null : introspector.getClassDeclaration(context.elementType);
...
${js}`;
    ...
    const lfs = link(memfsInstance, [["/node_modules", realNodeModules]]);
    const inputFs = new Union();
    inputFs.use(lfs);
    const outputVol = new Volume();
    const outputFs = createFsFromVolume(outputVol);
...
export { TemplateArchiveProcessor, TemplateMarkInterpreter, ensureDirSync, getTemplateClassDeclaration, nameUserCode, removeSync, writeFunctionToString };
